...
Tops.pack(side=TOP)
BOTTOMS = Frame(root,bg="pink",width = 1600,height=50)
...
def Ref():
    ...
    costofmeal = ('%.2f'% (costoffries +  costoflargefries + costofburger + costoffilet + costofcheeseburger + costofdrinks))
    #com = "Rs.", str('%.2f' % (costoffries + costoflargefries + costofburger + costoffilet + costofcheeseburger + costofdrinks))
    ...
    Service=('%.2f'% Ser_Charge)
    #Ser = "Rs.", str('%.2f' % Ser_Charge)
    OverAllCost=(PayTax + Totalcost + Ser_Charge)
    #oac = "Rs.", str(PayTax + Totalcost + Ser_Charge)
    PaidTax=('%.2f'% PayTax)
    #pt = "Rs.", str('%.2f' % PayTax)
    ...
    sql = f"insert into restaurant1 values ({rand.get()},{Fries.get()},{Largefries.get()},{Burger.get()},{Filet.get()},{Subtotal.get()},{Total.get()},{Service_Charge.get()},{Drinks.get()},{Tax.get()},{cost.get()},{Cheese_burger.get()})"
    print(sql)
    cur.execute(sql)
    cur.close()
    conn.commit()
...
status = Label(f2,font=('aria', 15, 'bold'),width = 16, text="-By Andrew",bd=2,relief=SUNKEN)
...
btnprice=Button(f1,padx=16,pady=8, bd=10 ,font=('ariel' ,16,'bold'),fg="black",width=10, text="PRICE", bg="powder blue",command=price)
